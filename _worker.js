...
export default {
	async fetch(request, env) {
		...
		let sni = "";
		let type = "ws";
		// ====== 【新增：通用参数透传与 Trojan 密码修复】 ======
	    // 1. 自动兼容传入的 password 参数
		let password = url.searchParams.get('pw') || url.searchParams.get('password');

		// 2. 打包所有不属于核心控制的未知参数
		let skipParams = ['host', 'uuid', 'pw', 'path', 'password', 'format', 'pn', 'port'];
		let remainingParams = [];
		...
		let extraQuery = remainingParams.join('&');
		if (extraQuery) extraQuery = '&' + extraQuery; 
		// ===================================================
		...
		if (!isSubConverterRequest && MamaJustKilledAMan.some(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead => userAgent.includes(PutAGunAgainstHisHeadPulledMyTriggerNowHesDead)) && MamaJustKilledAMan.length > 0) {
			...
			if ((noTLS == 'true' && 协议类型 == atob(`\u0056\u006b\u0078\u0046\u0055\u0031\u004d\u003d`)) || 协议类型 == 'VMess') {
				...
				notlsresponseBody = uniqueAddressesnotls.map(address => {
					...
					if (协议类型 == 'VMess') {
						...
						const 为烈士Link = `${atob(atob('ZG14bGMzTTZMeTg9')) + (uuid || password)}@${address}:${port}?security=&type=${type}&host=${host}&path=${encodeURIComponent(path)}&encryption=none${extraQuery}#${encodeURIComponent(addressid + EndPS)}`;
						return 为烈士Link;
					}
			...
			const responseBody = uniqueAddresses.map(address => {
				...
				if (协议类型 == 'VMess') {
					...
					const 特洛伊Link = `${atob(atob('ZEhKdmFtRnVPaTh2')) + (uuid || password)}@${address}:${port}?security=tls&sni=${sni}&alpn=${encodeURIComponent(alpn)}&fp=random&type=${type}&host=${伪装域名}&path=${encodeURIComponent(最终路径) + (scv == 'true' ? '&allowInsecure=1' : '')}&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}#${encodeURIComponent(addressid + 节点备注)}`;
					return 特洛伊Link;
				} else {
